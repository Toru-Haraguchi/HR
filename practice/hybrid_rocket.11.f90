...
!     Oxidizer_density Initial_oxidizer_mass-flow_rate
      real dens_oxi, m_dot_oxi_ini_1, m_dot_oxi_ini_2, m_dot_oxi_ini_3
	  real m_dot_oxi_ini_4
...
!     Pressure@chamber Burning_time Chamber_internal_radius
	  ...
	  real t_burn1, t_burn2, t_burn3

!      多数回燃焼
!.kana.1312
      real t_stop_burn,t_rest_burn,t_tot
      integer igon, dankai(0:nmax)
	  real t_not_burn
!.kana.1312_en
...
!     --- Calculated parameters ---
      ...
      real m_dot_fuel_ini,of1, of2, of3, of_ini
	  real of4 !再着火時のO/F
      real area_flow_ini
...
40    format(f16.3)
!------------------------------------------------------------------------------
      open(110,file='dv.inp',form='formatted')
        read(110,30) m_dot_oxi_ini_1
        ...
        read(110,30) m_pay
        read(110,30) mode_c   !.1.asoft, 2.soft
!!        read(110,30) t_burn
...
!       t_rest_burn=t_rest_burn+t_stop_burn
      ...
      open(111,file='cons.inp',form='formatted')
        read(111,*)  
        read(111,40) dt
        read(111,*)  
        read(111,*)  
...
!  ASOFTとSOFTは推進剤形状の決定方法が異なる．(engine-out.datを出力するまでがコード上での違い．)
!------------------------------------------------------------------------------
!     --- Calculated parameters ---
      r_port_ini = (m_dot_oxi_ini_1/(pi*go_ini))**0.5
...
!        print *, "alpha_fuel_ini=",alpha_fuel_ini
      ...
      r_dot_port_ini  = alpha_fuel_ini*a_fuel*go_ini**n_fuel
!        print *, "r_dot_port_ini=",r_dot_port_ini
      m_dot_fuel_ini  = m_dot_oxi_ini_1/of1
      m_dot_prop_ini  = m_dot_oxi_ini_1+m_dot_fuel_ini
!------------------------------------------------------------------------------
!     --- Engine Simulation ---
...
!!!4 ver11
      if(mode_c.eq.1)then
	      alpha_fuel_temp(0) = m_dot_oxi(0)/(2*pi*dens_fuel*l_fuel*a_fuel*of1*(m_dot_oxi(0)/pi)**n_fuel*r_port(0)**(-2*n_fuel+1))
	  elseif(mode_c.eq.2)then
	     alpha_fuel_temp(0) =1
	  endif
...
!org
        igon=0
        if(time_oxi(n).le.t_burn1)igon=1
        if(time_oxi(n).ge.t_burn2)igon=1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            if(time_oxi(n).le.t1)then
			   m_dot_oxi(n) = m_dot_oxi_ini_1
			   mn_oxi(n) = mn_oxi(n-1)+(m_dot_oxi(n-1)+m_dot_oxi(n))*dt/2.d0
			   
			elseif(time_oxi(n).gt.t1 .and. time_oxi(n).le.t1+t2)then
			   m_dot_oxi(n) = m_dot_oxi_ini_2
			   mn_oxi(n) = mn_oxi(n-1)+(m_dot_oxi(n-1)+m_dot_oxi(n))*dt/2.d0
			   
			elseif(time_oxi(n).gt.t1+t2 .and. time_oxi(n).le.t_burn1)then
			   m_dot_oxi(n) = m_dot_oxi_ini_3
			   mn_oxi(n) = mn_oxi(n-1)+(m_dot_oxi(n-1)+m_dot_oxi(n))*dt/2.d0
			   
		     elseif((time_oxi(n).gt.t_burn2) .and. (time_oxi(n).le.t_tot) .and. (t4.ne.0.d0))then
			    m_dot_oxi(n) = m_dot_oxi_ini_4
				mn_oxi(n) = mn_oxi(n-1)+(m_dot_oxi(n-1)+m_dot_oxi(n))*dt/2.d0
			endif
...
!     --- Time Integration ---
 77    l_fuel = l_fuel*l_factor
	     alpha_judge = 0.d0
		 
		 if(mode_c.eq.1)then
          if(l_factor.gt.1.0)then
		     alpha_fuel_temp(0) = m_dot_oxi(0)/(2*pi*dens_fuel*l_fuel*a_fuel*of1*(m_dot_oxi(0)/pi)**n_fuel*r_port(0)**(-2*n_fuel+1))
!        print *, "alpha_fuel_temp(",n,")=",alpha_fuel_temp(n)!!!!!!!!!!
			endif
		 elseif(mode_c.eq.1)then
		 	alpha_fuel_temp(0) = 1.
		 endif
 ...
 !       print *, dt
      do n=1,nmax
        time(n) = time(n-1) + dt
		time(n) = (nint(time(n) * 100.d0))/100.d0

!        if( n .le. 50)then
!        print *, time(n)
!        print *, int(time(n) * 100.0)
!		endif
...
!      if(time(n).le.t_burn)then
        ...
        if(time(n).lt.t_tot .and.  igon.eq.1)then
         ...
         if(time(n).lt.t1)then !第１ステップ
            r_port(n)          = r_port(n-1)+r_dot_port(n-1)*dt
!        print *, "r_port(",n,")=",r_port(n)
            ...
            if(mode_c.eq.1)then
			  alpha_fuel_temp(n)   =m_dot_oxi(n)/(2*pi*dens_fuel*l_fuel*a_fuel*of1*(m_dot_oxi(n)/pi)**n_fuel*r_port(n)**(-2*n_fuel+1))
			elseif(mode_c.eq.2)then
			  alpha_fuel_temp(n)   =1.
			endif
			
!        print *, "1alpha_fuel_temp(",n,")=",alpha_fuel_temp(n)
!        print *, "m_dot_oxi(",n,")=",m_dot_oxi(n)
!!	        if(alpha_fuel_temp(n) .gt. alpha_fuel_upper)then
!!			   alpha_fuel(n) = alpha_fuel_upper
!!               r_dot_port(n) = alpha_fuel(n)*a_fuel*go(n)**n_fuel
!!               m_dot_fuel(n) = r_dot_port(n)*dens_fuel*pi*2.0*r_port(n)*l_fuel
!!			   of(n) = m_dot_oxi(n)/m_dot_fuel(n)
!!			   check = of(n)
!!			else
!!			   alpha_fuel(n)   =m_dot_oxi(n)/(2*pi*dens_fuel*l_fuel*a_fuel*of1*(m_dot_oxi(n)/pi)**n_fuel*r_port(n)**(-2*n_fuel+1))
               of(n)                  = of1
			   r_dot_port(n) = alpha_fuel_temp(n)*a_fuel*go(n)**n_fuel
               m_dot_fuel(n) = r_dot_port(n)*dens_fuel*pi*2.0*r_port(n)*l_fuel
!!			endif
         ...
         elseif(time(n).lt.t1+t2 .and. time(n).ge.t1)then !第2ステップ
            r_port(n)          = r_port(n-1)+r_dot_port(n-1)*dt
            m_dot_oxi(n)   = m_dot_oxi_ini_2
            area_flow(n)    = pi*(r_port(n))**2.0
            go(n)                = m_dot_oxi(n)/area_flow(n)
            ...
            if(mode_c.eq.1)then
				alpha_fuel_temp(n)   =m_dot_oxi(n)/(2*pi*dens_fuel*l_fuel*a_fuel*of2*(m_dot_oxi(n)/pi)**n_fuel*r_port(n)**(-2*n_fuel+1))
			elseif(mode_c.eq.2)then
			  alpha_fuel_temp(n)   =1.
			endif
!        print *, "2alpha_fuel_temp(",n,")=",alpha_fuel_temp(n)!!!!!!!!!!
...
!!			   check = of(n)
!!			else
!!			   alpha_fuel(n)   =m_dot_oxi(n)/(2*pi*dens_fuel*l_fuel*a_fuel*of2*(m_dot_oxi(n)/pi)**n_fuel*r_port(n)**(-2*n_fuel+1))
               of(n)                  = of2
			   r_dot_port(n) = alpha_fuel_temp(n)*a_fuel*go(n)**n_fuel
               m_dot_fuel(n) = r_dot_port(n)*dens_fuel*pi*2.0*r_port(n)*l_fuel
!!			endif
         ...
         elseif(time(n).lt.t_burn1 .and. time(n).ge.t1+t2)then !第3ステップ
            r_port(n)          = r_port(n-1)+r_dot_port(n-1)*dt
            m_dot_oxi(n)   = m_dot_oxi_ini_3
            area_flow(n)    = pi*(r_port(n))**2.0
            go(n)                = m_dot_oxi(n)/area_flow(n)
			...
			if(mode_c.eq.1)then
			alpha_fuel_temp(n)   =m_dot_oxi(n)/(2*pi*dens_fuel*l_fuel*a_fuel*of3*(m_dot_oxi(n)/pi)**n_fuel*r_port(n)**(-2*n_fuel+1))
			elseif(mode_c.eq.2)then
			  alpha_fuel_temp(n)   =1.
			endif
 !       print *, "3alpha_fuel_temp(",n,")=",alpha_fuel_temp(n)!!!!!!!!!!
...
!!			   check = of(n)
!!			else
!!			   alpha_fuel(n)   =m_dot_oxi(n)/(2*pi*dens_fuel*l_fuel*a_fuel*of3*(m_dot_oxi(n)/pi)**n_fuel*r_port(n)**(-2*n_fuel+1))
              of(n)                  = of3
			   r_dot_port(n) = alpha_fuel_temp(n)*a_fuel*go(n)**n_fuel
               m_dot_fuel(n) = r_dot_port(n)*dens_fuel*pi*2.0*r_port(n)*l_fuel
!!			endif
            ...
            elseif(time(n).ge.t_burn1 .and. time(n).lt.t_burn2)then
            r_dot_port(n) = 0.d0
            r_port(n)     = r_port(n-1)
            m_dot_oxi(n)  = 0.d0
            go(n)         = 0.d0
            ...
            if(mode_c.eq.1)then
            alpha_fuel_temp(n) = 0.d0
            elseif(mode_c.eq.2)then
			  alpha_fuel_temp(n)   =1.
			endif
			
 !       print *, "4alpha_fuel_temp(",n,")=",alpha_fuel_temp(n)!!!!!!!!!!
						   
         elseif(time(n).ge.t_burn2 .and. time(n).lt.t_tot .and. t4.ne.0.d0)then !再着火
            r_port(n)          = r_port(n-1)+r_dot_port(n-1)*dt
            m_dot_oxi(n)   = m_dot_oxi_ini_4
            area_flow(n)    = pi*(r_port(n))**2.0
            go(n)                = m_dot_oxi(n)/area_flow(n)
!        print *, "5alpha_fuel_temp(",n,")=",alpha_fuel_temp(n)!!!!!!!!!!			
!!	        if(alpha_fuel_temp(n) .gt. alpha_fuel_upper)then
!!			   alpha_fuel(n) = alpha_fuel_upper
!!              r_dot_port(n) = alpha_fuel(n)*a_fuel*go(n)**n_fuel
!!               m_dot_fuel(n) = r_dot_port(n)*dens_fuel*pi*2.0*r_port(n)*l_fuel
...
!!			   alpha_fuel(n)   =m_dot_oxi(n)/(2*pi*dens_fuel*l_fuel*a_fuel*of3*(m_dot_oxi(n)/pi)**n_fuel*r_port(n)**(-2*n_fuel+1))
            ...
            if(mode_c.eq.1)then
			  alpha_fuel_temp(n)   =m_dot_oxi(n)/(2*pi*dens_fuel*l_fuel*a_fuel*of4*(m_dot_oxi(n)/pi)**n_fuel*r_port(n)**(-2*n_fuel+1))
			elseif(mode_c.eq.2)then
				alpha_fuel_temp(n)   =1.
			endif
...
!     after engine cut off
            ...
            m_oxi(n)      = m_oxi(n-1)
            mn_fuel(n)    = mn_fuel(n-1)
!					  print *,'AAA r_port',n,' = ',r_port(n)	
       endif
	   
	   if(alpha_fuel_temp(n) .gt. alpha_judge)then
	      alpha_judge = alpha_fuel_temp(n)
	   endif   
!		  print *,'r_port',n,' = ',r_port(n)	   
       enddo
	   
	   if(alpha_judge .gt. 3.d0)then  !alphaが基準(3)を超える場合は再設計を行う
	      l_factor = alpha_judge/3.d0
!		  print *,'alpha_judge = ',alpha_judge
...
!     --- Output ---
      open(2,file='engine-out.dat',form='formatted')
        write(2,*)'time ','propellant_mass ',&
          &'m_dot_oxi ','m_dot_fuel ','O/F ',&
          &'m_oxi ','m_fuel ',&
          &'m_dot_prop ','r_port ','Go ',&
          &'r_dot_port ','flow_area ','alpha_fuel_temp '

        do i=0,nmax
          write(2,*)time(i),m_prop(i),m_dot_oxi(i),&
          &m_dot_fuel(i),of(i),m_oxi(i),m_fuel(i),&
          &m_dot_prop(i),r_port(i),go(i),&
          &r_dot_port(i),area_flow(i),alpha_fuel_temp(i)

        enddo
      close(2)
!		print*, of1, pc_bar, epsilon
!------------------------------------------------------------------------------
!     --- NASA-CEA(time=0) ---
...
!       accn2 = sin(theta_l0*pi/180.)*(thrust(n)-drag(n))/m_tot(n)-gravity
...
!        accn2c = cos(theta_l0*pi/180.)*(thrust(n)-drag(n))/m_tot(n)
      elseif(vel1c.lt.0.0)then
       accn2c = ((abs(cos(theta_p1*pi/180.))*thrust(n))&
	   &      +(drag(n)*abs(cos(theta_l1*pi/180))))/m_tot(n)
!        accn2c = cos(theta_l0*pi/180.)*(thrust(n)+drag(n))/m_tot(n)
...
!     --- Accel of theta_p ---
!      print *,hein2
	   if(hein2 .lt. l_tot*1.2*sin(theta_ini*pi/180.0))then !!!!!! rauncher 
	      acc_theta_pn2 = 0.
		  omegan2 = 0.
		  theta_pn2 = theta_p1
		  theta_ln2 = theta_l1
